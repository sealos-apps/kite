...
- Offline Helm chart support discovers Helm OCI charts from a configured registry repository prefix (`KITE_HELM_OCI_REGISTRY_BASE`, for example `oci://hub.example.local/kite-helm`). Kite may scan only that configured prefix and must not perform unbounded registry-wide exposure. Registry credentials, plain HTTP, insecure TLS, and CA file settings are server-side runtime options only and must not be embedded in chart URLs or exposed in API responses. When Artifact Hub is disabled, keep the backend proxy, frontend fallback, AI tool resolution, and scheduled auto-upgrade paths from calling Artifact Hub. Offline container images may use the same registry host, but keep them outside the chart prefix under their normal repository paths such as `hub.example.local/bitnami/nginx:<tag>`. For OCI catalog releases with offline images enabled, Kite injects offline image defaults, dry-runs, and blocks writes if rendered workload images point outside the configured registry; image mirroring remains a preparation step and is not performed during install.
