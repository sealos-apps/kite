...
- AI Agent UI is enabled by default for new installs, but chat runtime still requires a configured API key. The `/settings` entry should be visible to authenticated users; only Kite administrators can edit the first AI Agent-only tab. Keep kubectl, node terminal, analytics, and login-prompt controls out of that visible panel unless the product decision changes explicitly.
