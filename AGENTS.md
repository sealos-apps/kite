...
- Offline Helm chart support uses a declared static OCI catalog (`KITE_HELM_OCI_CATALOG` or `KITE_HELM_OCI_CATALOG_FILE`) that points at charts already mirrored into an offline OCI registry; Kite must not auto-scan the registry for charts. OCI catalog URLs are returned by authenticated chart read APIs, so reject credentials, query parameters, fragments, mismatched version tags, and digest-only references. OCI registry access settings such as username, password, plain HTTP, insecure TLS, and CA file are server-side runtime options only and must not be embedded in catalog URLs or exposed in API responses. When Artifact Hub is disabled, keep the backend proxy, frontend fallback, AI tool resolution, and scheduled auto-upgrade paths from calling Artifact Hub.
