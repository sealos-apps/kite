...
- **KITE_NAMESPACE_SCOPE_EXEMPT_NAMESPACES**：逗号分隔的命名空间白名单（例如 `ns-admin,platform-admin`）。命中后会跳过 kubeconfig `current-context.namespace` 导致的 namespace-scope 锁定，并且 Sealos SSO 自动生成的角色会被授予该集群下 `*` 命名空间权限。仅应配置那些实际具备全局/集群管理员权限的命名空间。
